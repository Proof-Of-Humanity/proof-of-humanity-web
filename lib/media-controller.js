...
export async function loadFFMPEG() {
  if (!ffmpeg || !ffmpeg.isLoaded()) await getFFmpegInstance();
}
...
export async function videoSanitizer(
  ...
  try {
    ...
    ffmpeg.setProgress((progress) => {
      if (progress.time !== undefined) callback(progress.time / duration);
    });
    ...
    if (mirrored) options.push("-vf", "hflip", "mpdecimate");
    else options.push("-vf", "mpdecimate");
...
export async function photoSanitizer(buffer) {
  try {
    ...
    if (isGrayscale) throw "image_grayscale";
