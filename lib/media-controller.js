...
export async function videoSanitizer(inputBuffer, size, OS, callback, mirrored,duration) {
  try {
    await loadFFMPEG();

    ffmpeg.setProgress((progress ) => {
      console.log("ratio",progress.time/duration)
      if(progress.time/duration<1){
...
export async function photoSanitizer(buffer) {
  try {
    ...
    const imageToJpg = await image.quality(95).resize(width,height).getBufferAsync(Jimp.MIME_JPEG);
